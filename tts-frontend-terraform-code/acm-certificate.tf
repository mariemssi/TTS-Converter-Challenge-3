...
# The validation is done after the creation of the certificate. It can take some time.
...
# In the validation of the AWS ACM SSL certificate, DNS records are created To verify that you have control over the domain for which you are requesting the SSL certificate.
...
resource "aws_route53_record" "cert_validation" {
  ...
  allow_overwrite = true //The allow_overwrite argument is set to true, which means that the resource can overwrite existing records with the same name
